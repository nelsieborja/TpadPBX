...
(function(d) {
	...
	function w(g, a, b, c, e, f, d, i) {
		var h = [];
		return f > e && f - e > n / 2 + 1.0E-4 ? (h = h.concat(w(g, a, b, c, e, e + n / 2, d, i)), h = h.concat(w(g, a, b, c, e + n / 2, f, d, i))) : f < e && e - f > n / 2 + 1.0E-4 ? (h = h.concat(w(g, a, b, c, e, e - n / 2, d, i)), h = h.concat(w(g, a, b, c, e - n / 2, f, d, i))) : (h = f - e, ["C", g + b * m(e) - b * B * h * l(e) + d, a + c * l(e) + c * B * h * m(e) + i, g + b * m(f) + b * B * h * l(f) + d, a + c * l(f) - c * B * h * m(f) + i, g + b * m(f) + d, a + c * l(f) + i])
	}
	...
	d.SVGRenderer.prototype.cuboidPath = function(d) {
		...
		return [
			["M", a[0].x, a[0].y, "L", a[1].x, a[1].y, "L", a[2].x, a[2].y, "L", a[3].x, a[3].y, "Z"], a[7].y < a[1].y ? d : a[4].y > a[2].y ? b : [], a[6].x > a[1].x ? c : a[7].x < a[0].x ? e : [], (a[0].z + a[1].z + a[2].z + a[3].z) / 4, h > 0 ? (a[0].z + a[7].z + a[6].z + a[1].z) / 4 : (a[3].z + a[2].z + a[5].z + a[4].z) / 4, i > 0 ? (a[1].z + a[2].z + a[5].z + a[6].z) / 4 : (a[0].z + a[7].z + a[4].z + a[3].z) / 4
		]
	};
	d.SVGRenderer.prototype.arc3d = function(g) {
		...
		a.animate = function(a, b, c) {
			a.end || a.start ? (this._shapeArgs = this.shapeArgs, d.SVGElement.prototype.animate.call(this, {
				_args: a
			}, {
				duration: b,
				step: function() {
	...
	d.wrap(d.Axis.prototype, "render", function(d) {
		...
		if (this.chart.is3d()) {
			...
			if (this.horiz) this.axisLine && this.axisLine.hide(), f = {
					x: l,
			...
			if (d.svg)
				if (b === !0 && (b = {}), a) {
					if (e.oldtranslateX = e.translateX, e.oldtranslateY = e.translateY, a = {
						...
						scaleX: 0.001,
						scaleY: 0.001
					}, e.attr(a), f) f.attrSetters = e.attrSetters, f.attr(a)
				}
				else a = {
					...
					scaleX: 1,
					scaleY: 1
				}, e.animate(a, b), f && f.animate(a, b), this.animate = null
		}
		else g.apply(this, [].slice.call(arguments, 1))
	});
